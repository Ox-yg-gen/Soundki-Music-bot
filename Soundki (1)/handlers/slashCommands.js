...
const dirSetup = [{
			...
			"CmdDescription": "Grant specific Information about the Bot!"
	},{
			"Folder": "Settings", "CmdName": "settings",
			"CmdDescription": "Adjust Settings of the Bot!"
	},{
			"Folder": "Music", "CmdName": "music",
			"CmdDescription": "Start Playing and Queuing Music"
	},{
				...
				if(cmdSetup && cmdSetup.Folder) {
					...
					for (let file of slashCommands) {
						...
						if (pull.name && pull.description) {
								...
								if(pull.options && pull.options.length > 0){
									for(const option of pull.options){
										if(option.User && option.User.name && option.User.description){
											...
											console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(`A Option is missing the Name or/and the Description of ${pull.name}`));
										}
									}
								}
								return subcommand;
							...
							console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(` ${file} error -> missing a help.name, or help.name is not a string.`));
							continue;
						}
					}
					...
					return console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(`The Subcommand folder ${dir} is not in the dirSetup configuration!`));
				}
			} else {
		...
		client.on("ready", () => {
			if(config.loadSlashsGlobal){
				...
				.then(slashCommandsData => {
					...
					console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`} Loaded for all: ${`Possible Guilds`}`));
					console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(`Because u are Using Global Settings, it can take up to 1 hour until the Commands are changed!`));
				}).catch((e)=>{});
			} else {
				client.guilds.cache.map(g => g).forEach(async (guild) => {
					try{
						...
						.then(slashCommandsData => {
							client.slashCommandsData = slashCommandsData;
							console.log(chalk.bold(chalk.blue.bold(`[Soundki]`)) + chalk.cyan.bold(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`} Loaded for: ${`${guild.name}`}`));
						}).catch((e)=>{});
					}catch (e){
		...
		client.on("guildCreate", async (guild) => {
			try{
				if(!config.loadSlashsGlobal){
					...
					guild.commands.set(allCommands)
					.then(slashCommandsData => {
					console.log(`[Soundki] ${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`} Loaded for: ${`${guild.name}`.underline}`); 
