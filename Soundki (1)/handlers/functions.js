...
function onCoolDown(message, command) {
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     .setColor(config.colors.waves)]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
     if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
   
     let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("891379368037859409").setLabel("Back")
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           return b.reply(`<:bluewaves:891379368037859409> **Only the one who typed ${prefix}help is allowed to react!**`, true)
           //page forward
           if(b.customId == "1") {
