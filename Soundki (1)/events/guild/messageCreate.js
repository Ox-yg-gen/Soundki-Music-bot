...
module.exports = async (client, message) => {
    ...
    client.settings.ensure(message.guild.id, {
      ...
      botchannel: [],
      color: "#cf20fb",
      react: "<:soundki:920739390484021308>",
    ...
    if (command) {
      ...
      return message.reply({ content: `**${emoji} Command not allowed in this channel` }).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(() => {});
      ...
      if(!botchannels || !Array.isArray(botchannels)) botchannels = [];
        ...
        try {
          ...
          if(command.minargs && command.minargs > 0 && args.length < command.minargs) {
            return message.reply({embeds: [new MessageEmbed()
              .setColor(color)
              .setTitle(`${emoji} WRONG COMMANDS USAGE!`)
