...
module.exports = {
	...
	run: async (client, interaction) => {
		try {
			...
			await interaction.reply({ content: `${emoji} **Searching...** \`${Text}\``, ephemeral: false });
			try {
			...
			//Edit the reply
			interaction.editReply({ content: `${queue?.songs?.length > 0 ? " " + emoji + " **Skipping to:**" : " " + emoji + " **Now Playing:**"} \`${Text}\``, ephemeral: true });
			} catch (e) {
