...
module.exports = {
	...
	run: async (client, message, args) => {
		try {
			...
			new MessageEmbed()
			.setColor(color)
			.setTitle(`${emoji} ADD A POSITION DURATION!`)
			.setDescription(`**Usage:** - \`${client.settings.get(guild.id, "prefix")}seek <50>\`\nAnd that would seek to 50s of the song`)
			],
			})
			}
			...
			new MessageEmbed()
			.setColor(color)
			.setTitle(`${emoji} NOT A VALID NUMBER!`)
